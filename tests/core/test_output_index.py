...
def test_default_selection_excludes_hetatm(topology: MinimalTopology) -> None:
    ...
    assert output_index.atom_mask.tolist() == [True, True, True, True, True, False]
    assert output_index.probe_mask.tolist() == [True, True, True, True, True, False]
    assert output_index.output_mask.tolist() == [True, True, False, True]
...
def test_include_hetatm_in_environment_only(topology: MinimalTopology) -> None:
    ...
    assert output_index.atom_mask.tolist() == [True, True, True, True, True, True]
    assert output_index.probe_mask.tolist() == [True, True, True, True, True, False]


def test_chain_filtering(topology: MinimalTopology) -> None:
    output_index = OutputIndex.from_selection(topology, include_chains=["B"])
    assert output_index.atom_mask.tolist() == [False, False, False, False, True, False]
    assert output_index.output_res_idx.tolist() == [3]
...
def test_custom_atom_mask_override(topology: MinimalTopology) -> None:
    ...
    assert output_index.atom_idx.tolist() == [2]
    assert output_index.probe_idx.tolist() == [2]
    assert output_index.output_res_idx.tolist() == [1]
